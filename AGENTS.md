...
- **Seeded RNG only**: Never call `Math.random()` directly. You must use the shared `rng` instance exported from `src/core/rng.ts` to ensure gameplay is deterministic.
...
| Use `z.record(z.string(), z.unknown())` for typed params | Use `z.discriminatedUnion('type', [...])` with explicit shapes per variant. If the runtime has a `switch`, the schema needs a union. |
| Hardcode editor dropdown values               | Derive dropdown options from canonical enums, registries, or `CampaignData` keys programmatically                                   |
...
- **Determinism in Procedural Generation:** Never use `Date.now()` or `Math.random()` in a roguelike, even for string ID generation. This permanently breaks the ability to reliably share seeds. Everything must be tied to deterministic state counters (`nextQuestId`) or the seeded `rng`.
...
- **UI Controller vs View Separation:** UI components should be strictly split between DOM manipulation/state reading (Views) and DOM Event generation/Intents routing (Controllers). Monolithic files that contain both `window.addEventListener('keydown')` logic alongside massive `document.createElement` layouts eventually create impenetrable spaghetti. Top-level files like `src/main.ts` should remain a thin entry point for wiring, deferring specific mapping logic to dedicated handlers like `src/core/input_handler.ts` and UI views to `src/rendering/ui/`.
- **Flat is Better than Nested:** In complex systems that handle multiple conditions or types (like parsing intents, resolving events, or applying trigger consequences), use guard clauses (early returns or `break`s) and `switch` statements to keep the logic flat. Avoid deeply nested `if/else` chains, as they make state mutations difficult to follow and increase the cognitive load.
- **Zod Schemas Must Mirror Runtime Switches:** If the runtime code has a `switch` on a `type` string (e.g., `evaluateCondition()` switching on `condition.type`), the corresponding Zod schema **must** use a `z.discriminatedUnion('type', [...])` with explicit param shapes per variant — never `z.record(z.string(), z.unknown())`. Opaque param bags make editor UIs fall back to raw JSON textareas, prevent compile-time exhaustiveness checking, and allow silent typos in param keys. The schema is the single source of truth for both the editor form renderer and the runtime system.
- **Editor Dropdowns Must Derive from Canonical Sources:** Never maintain a handwritten list of options in editor UI code when a canonical enum, registry, or `CampaignData` key already exists as the source of truth. Hardcoded dropdown lists silently diverge from the runtime as new variants are added, leaving designers unable to access half the available options. Always derive dropdowns programmatically from the authoritative type definition.
- **Tag-Based Interactions over Item-Specific Logic:** When building data-driven interaction systems (like item reactions, crafting recipes, or conditional triggers), always operate on **tags** (category-level identifiers) rather than specific item IDs. Item-specific rules recreate NetHack's maintenance nightmare in JSON form. Tag-based rules scale combinatorially — adding one new tag to an item automatically grants it all existing interactions for that tag category.
- **Never Duplicate Reference-Resolution Logic:** If two UI renderers both need to resolve cross-registry references (e.g., populating a dropdown of entity IDs for a `targetId` field), extract the resolution logic into a single shared utility function. Duplicated `getReferenceOptions()`-style functions silently diverge as new reference types are added, causing one renderer to show dropdowns while the other shows raw text inputs for the same field.
